...
class TripletNode(Node):
    ...
    def find_argument(self, type):
        outlinks = self._get_out_links_list(type="argument", attr__argument_type=type)
        if len(outlinks) > 0:
    ...
    def from_subject_predicate_object(np_subject, vp_predicate, np_object):
        ...
        if np_subject:
            links.append(
                Link.create("argument", triplet, np_subject, argument_type="subject"))
        links.append(Link.create("argument", triplet, vp_predicate, argument_type="predicate"))
        if np_object:
            links.append(Link.create("argument", triplet, np_object, argument_type="object"))
    ...
    def from_subject_predicate_attr(np_subject, vp_predicate, advp_attr):
        ...
        links.append(Link.create("argument", triplet, vp_predicate, argument_type="predicate"))
        if advp_attr:
            links.append(Link.create("argument", triplet, advp_attr, argument_type="attribute"))
